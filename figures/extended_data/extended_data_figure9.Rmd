...
## These are 
telseq_joint <- read.table(paste0(main_repo_path,"/data/telseq_estimates_wgs_ff.tsv"), sep = "\t", header = TRUE)
mono_poly <- read.table(paste0(main_repo_path,"/data/clonal_count_estimation/clonal_SNVs_WES_WGS.csv"), sep = ",", header = TRUE) %>% 
  filter(has_WGS == "True") %>% mutate(Group = ifelse(is_poly_WGS == "True", "Poly", "Mono")) %>%  select(Tumor_Sample_Barcode, Group)

telseq_joint <- merge(telseq_joint, mono_poly, by="Tumor_Sample_Barcode")
...
## save the plot
ggsave(paste0(output_path,"/ED_Figure_9.pdf"), telseq_patients.plt, width = 8.5, height = 6)
...
for (stage in unique(telseq_joint$Stage)) {
  ...
  for (patient in unique(telseq_joint$Patient)) {
    ...
    if (length(unique(subset$Group)) == 2) { 
      ...
      # there should be at least three data points to calculate the p-value
      if (length(mono) > 2 & length(poly) > 2) { 
